...
_SPECS = [
    ...
    _cmd("restart-window", [], children=("restart_window_action", (
        _cmd("open", [
            _arg("--reason", help="Why the restart is happening (recorded on the window)"),
            _arg("--task-id", help="Task id of the worker performing the restart, if any"),
        ...
        _cmd("close", [
            _arg("window_id", nargs="?", type=int,
                 help="Window id to close (default: your own open window)"),
        ], help="Close an open restart window (by id, or your own open window)"),
        _cmd("list", [
            _arg("--all", action="store_true",
                 help="Include closed/expired windows, not just open ones"),
        ], help="List restart windows (open first)"),
