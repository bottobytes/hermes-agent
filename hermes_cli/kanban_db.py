...
def _text(value, *, field: str = "value") -> str:
...
class Task:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            **{col: row[col] for col in _TASK_REQUIRED_COLUMNS if col not in ("title", "body")},
            title=_text(row["title"]),
            body=_text_opt(row["body"]),
            result=_text_opt(row["result"]),
            **{col: g(col) for col in _TASK_OPTIONAL_COLUMNS if col not in ("body", "result")},
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "completion_contract" not in cols:
        # Structured acceptance contract for goal_mode workers (upstream
        # addition post-v2026.8.27). NULL = no contract.
        _add_column_if_missing(
...
def promote_task(
    ...
    # (``triage`` accepted per t_45f1fcc8 as the structured exit for a block-loop
    # escalation an operator has reviewed — same rationale as unblock_task.)
    parents = conn.execute(
...
# Sportacus (t_bbdec489): keep OUR init_db — the ghost-board refusal guard
# (BoardRemovedError on a removed slug with a stale init-cache entry) is
# origin-resident here and must not be shadowed by the split module's
# re-export above. Our connect() keeps the same guard.
_init_db_guarded = init_db
def init_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> Path:
    if db_path is None and board is not None:
        slug_guard = _normalize_board_slug(board)
        if (
            slug_guard
            and slug_guard != DEFAULT_BOARD
            and (slug_guard in _REMOVED_BOARD_SLUGS or str(kanban_db_path(board=board).resolve()) in _INITIALIZED_PATHS)
            and _board_is_gone(slug_guard)
        ):
            raise BoardRemovedError(
                f"board {slug_guard!r} was removed but this process "
                f"still had it cached; refusing to resurrect it."
            )
    return _init_db_guarded(db_path, board=board)
