...
class DispatchResult:
    ...
    """Task ids flagged by the unreachable-assignee sweep (t_09c47c8d):
    parked in ready/review/todo/triage under an assignee that names no
    Hermes profile. Subscribers are woken with ``assignee_unreachable``."""
    review_spawn_starved: list[str] = field(default_factory=list)
    """Review-lane task ids under a VALID profile whose reviewer never
    spawned past the starvation threshold (t_f0393d9f)."""
    respawn_starved: list[str] = field(default_factory=list)
    """Ready-lane task ids under a VALID profile silently deferred past the
    starvation threshold (t_67d80b05) — respawn-guard or another silent
    defer refusing to spawn while the card looks healthy."""
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    Sportacus hardening: delegates to the enriched classifier in
    ``hermes_cli.kanban_db`` (provider-cause death classification,
    rate-limit sentinel sidecar consumption, restart-window neutral
    requeues, restart-orphan epoch adoption, verdict-suspect detection —
    t_d4c215a7/t_e586ea59/t_39ece1cd/t_09c47c8d), which carries the full
    side-channel set (``_last_auto_blocked`` / ``_last_rate_limited`` /
    ``_last_restart_killed`` / ``_last_restart_orphans``) that
    ``dispatch_once`` reads to populate ``DispatchResult``.
    """
...
def _run_reclaim_phase(
    ...
    *,
    ...
    # restart-affected tasks went back to ``ready`` without counting a
    # failure — surface the neutral lanes for telemetry / tests.
    result.auto_blocked.extend(getattr(detect_crashed_workers, "_last_auto_blocked", []))
    result.rate_limited.extend(getattr(detect_crashed_workers, "_last_rate_limited", []))
    result.restart_killed.extend(getattr(detect_crashed_workers, "_last_restart_killed", []))
    result.restart_orphans.extend(getattr(detect_crashed_workers, "_last_restart_orphans", []))
    result.timed_out = enforce_max_runtime(conn)
    result.promoted = _kb.recompute_ready(conn, failure_limit=failure_limit)
    # --- Sportacus hardening sweeps (passive repair; never mutate assignment) ---
    # Terminal-event reconciler: runs on the same flag because both
    # reconcilers are passive repair passes over existing state.
    try:
        _kb.reconcile_missing_terminal_events(conn)
    except Exception:
        _kb._log.exception("kanban dispatcher: terminal-event reconciler failed")
    ...
    # profile. A hallucinated/typo'd assignee is silently terminal without
    # this pass — no run, no wake, no error. Emits ``assignee_unreachable``
    # (notifier-terminal on both wake paths) + a board comment.
    try:
        result.flagged_unreachable = _kb.reconcile_stuck_assignees(conn)
    except Exception:
        _kb._log.exception("kanban dispatcher: unreachable-assignee sweep failed")
    # Review-spawn starvation detector (t_f0393d9f): a review card under a
    # REAL profile that never spawns (per-profile cap, crash loop,
    # review-lane wedge) produces zero events and is invisible. Emits
    # ``review_spawn_starved`` + a board comment; idempotent per episode.
    try:
        result.review_spawn_starved = _kb.detect_review_spawn_starved(conn)
    except Exception:
        _kb._log.exception("kanban dispatcher: review-spawn-starved detector failed")
    # Ready-lane starvation detector (t_67d80b05): READY cards the dispatcher
    # keeps silently deferring — respawn-guard blockers write per-tick
    # respawn_guarded events that no notifier terminal set carried (the
    # t_c5b9190f incident: ~20h, 696 guard events, zero wakes).
    try:
        result.respawn_starved = _kb.detect_respawn_starved(conn)
    except Exception:
        _kb._log.exception("kanban dispatcher: respawn-starved detector failed")
...
def _dispatch_once_locked(
    ...
    *,
    ...
    # Restart-window spawn hold (Sportacus hardening): a worker (or operator)
    # has declared a container/host restart. Spawning now would create workers
    # the restart kills seconds later — each burning a claim and a crash sweep.
    # Hold spawns while the window is open; the requeued cards resume on the
    # first tick after the window closes (or expires via its TTL).
    if _kb.restart_window_holds_spawns(conn):
