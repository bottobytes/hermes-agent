...
class CLIChatTurnMixin:
    ...
    def _chat_settle_turn(self, turn):
        ...
        # failure_retryable, set by the turn-recovery exhaustion path) so
        # single-query callers can map a transient provider throttle to the
        # kanban EX_TEMPFAIL exit instead of a generic failure. Best-effort:
        # a non-dict result simply clears the stash. (t_d4c215a7)
        self._last_turn_failure_reason = (
            turn.result.get("failure_reason") if isinstance(turn.result, dict) else None
        )
        self._last_turn_failed = bool(isinstance(turn.result, dict) and turn.result.get("failed"))
