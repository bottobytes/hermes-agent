...
def _cmd_block(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        def _block_op(tid):
            ...
            # dependency block) — capture them so the fail line prints WHY and
            # what to do instead of a bare "cannot block".
            ok, block_reason = kb.block_task(
                conn, tid, reason=reason, kind=kind,
                expected_run_id=_worker_run_id_for(tid), with_reason=True)
            _block_reasons[tid] = block_reason
            return ok

        _block_reasons: dict[str, Optional[str]] = {}
        op = _commented(conn, reason, author, "BLOCKED", _block_op)
        def _block_fail(tid):
            why = _block_reasons.get(tid)
            return f"cannot block {tid}: {why}" if why else f"cannot block {tid}"
        return _bulk_apply(ids, op, ok_msg, _block_fail)
