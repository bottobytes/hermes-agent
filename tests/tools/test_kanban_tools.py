...
def worker_env(monkeypatch, tmp_path):
    ...
    # in tests/hermes_cli/conftest.py — but vouch ONLY for the synthetic
    # names this suite actually uses, so tests that rely on real filesystem
    # resolution (e.g. the upstream #106163 unknown-reviewer rejection)
    # still see the truth for every other name.
    import hermes_cli.profiles as _profiles
    _real_profile_exists = _profiles.profile_exists
    def _house_profile_exists(name):
        if str(name) in ("test-worker", "peer", "qa", "factory"):
            return True
        return _real_profile_exists(name)
    monkeypatch.setattr(_profiles, "profile_exists", _house_profile_exists)
...
def test_request_review_rejects_unknown_reviewer_without_mutation(monkeypatch, worker_env, tmp_path):
    ...
    with kbc.connect() as conn:
        ...
        # t_fbd0fb38 stateful gate: the rejection records an audit trail
        # (``assignee_invalid_attempt`` strike counter) but never mutates the
        # task's lifecycle events.
        after_events = kb.list_events(conn, worker_env)
        lifecycle = [e for e in after_events if e.kind != "assignee_invalid_attempt"]
        assert lifecycle == before_events
