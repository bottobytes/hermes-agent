            ...
            try:
                ...
                if not _kanban_dispatch_allowed():
                    ...
                    for _slug, _res in (results or []):
                        if _res is None:
                            continue
                        _unreach = list(getattr(_res, "flagged_unreachable", None) or [])
                        _starved = list(getattr(_res, "review_spawn_starved", None) or [])
                        _rstarved = list(getattr(_res, "respawn_starved", None) or [])
