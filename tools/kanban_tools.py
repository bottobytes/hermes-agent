...
def _handle_block(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        # t_17cda1e8: kernel refusal reasons are actionable (e.g. a vacuous
        # dependency block) — surface them verbatim so the worker can correct
        # the call instead of retrying it blind.
        ...
        _check(ok, f"could not block {tid}: "
                   f"{block_reason or 'unknown id or not in running/ready'}")
...
def _handle_request_review(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        # Write-time phantom-reviewer gate (t_fbd0fb38, supersedes the stateless
        # #106163 check): reject BEFORE any state changes so a hallucinated
        # name can never park the card in review under an unspawnable assignee.
        # 3-strike rule: the third invalid name auto-falls back to reviewer=None
        # routing (self/FO review) with a visible card comment, so validation
        ...
        # durable reset marker — one good write clears the strike counter so
        # a card that learned the right name isn't one typo from forced
        # fallback on its next review cycle.
        with kb.write_txn(conn):
