...
_ZAI_POLICY_NOTES = {
    ...
    "transient_throttle_long": " (transient throttle adaptive long backoff)",
    "transient_throttle_short": " (transient throttle short retry)",
}


def compute_error_backoff(
    agent: Any, api_error: Exception, *, retry_count: int, max_retries: int, is_rate_limited: bool,
    is_zai_coding_overload: bool, base_url: Any, model: Any, failure_reason: Any = None,
) -> float:
    ...
    # Throttle family: rate limits, Z.AI coding overloads, AND the generic
    # transient-throttle set (provider overload / upstream 429 on any
    # provider) — the adaptive policy widens waits for the whole family so a
    # ~90s overload burst cannot burn the retry budget on hot-loop retries.
    _adaptive = is_rate_limited or is_zai_coding_overload or is_transient_throttle_reason(failure_reason)
    ...
    if _adaptive:
        ...
        if not _policy_note and _retry_after is not None:
        ...
        # Long throttle waits (minutes) must surface immediately instead of
        # making the TUI look frozen.
        if _backoff_policy in ("zai_coding_overload_long", "transient_throttle_long") or (
            _retry_after is not None and _retry_after >= 30
        ):
...
def route_classified_error(
    ...
    # Generic transient throttle (rate limit / overload / upstream 429 on ANY
    # provider): same deepening of the retry budget so the adaptive long-backoff
    # tier (15/30/60/60) is reachable. Sized so a ~90s provider overload burst
    # can no longer exhaust the loop at the default budget of 3 — the turn now
    # waits it out, and if it still fails the terminal result carries the
    # failure_reason for the worker exit-code mapping.
